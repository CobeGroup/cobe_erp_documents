...
| **`is_compensatory`** (Is Compensatory) | **Nghỉ bù** — **KHÔNG** có Leave Allocation, số dư trên Desk **âm là bình thường** (dùng chung `allow_negative`). Quyền nghỉ nằm ở **QUỸ GIỜ** (`attendance/comp_leave_ledger.py`): giờ trên **HR Overtime Request đã duyệt** quy đổi payout **"Nghỉ bù"** cộng dồn vào quỹ, đơn nghỉ trừ quỹ theo **4h = 0,5 ngày / 8h = 1 ngày** (validate `_validate_comp_leave_balance`), trừ FIFO theo hạn dùng. Lô giờ hết hạn cuối kỳ 30/06 & 31/12. HR chỉnh tay bằng chứng từ **HR Comp Leave Adjustment**. Loại này **luôn hiện trong dropdown** của NV (fallback trong code). Vd "Nghỉ bù". |
