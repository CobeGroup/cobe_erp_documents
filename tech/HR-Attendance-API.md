...
- **Miễn trừ của bộ đếm** (đổi 21/09/2026) — `utils/hr_exemption.exempt_for`: HR Manager / HR User /
  ...
  cổng chỉ đọc role người lưu, nên người giữ role HR tự cấp cho mình bao nhiêu cũng lọt. Luật này là
  MỘT hàm dùng chung cho hai cửa: hạn mức ngày khai bù (`_validate_monthly_quota`) và quỹ giờ Nghỉ bù
  (`overrides/leave_application._validate_comp_leave_balance`). Hạn NỘP (`_validate_filing_deadline`)
  giữ miễn trừ theo role như cũ: quá hạn là chuyện thời điểm, sửa hộ lúc nào cũng phải mở, còn hạn
  mức là chuyện chia phần.
- **KHÔNG tắt `allow_negative` của Leave Type `Nghỉ bù`.** Nó trông như lưới an toàn thứ hai nhưng
  thực ra là điều kiện để mô hình sổ giờ chạy: sổ cố ý bỏ `Leave Allocation` (đo 21/09/2026 — toàn hệ
  thống có **1** allocation cho `Nghỉ bù`, so với 255 của `Phép Năm`). Tắt cờ đó thì HRMS
  `validate_dates_across_allocation` đòi phải có allocation phủ ngày nghỉ và `show_insufficient_
  balance_message` chuyển từ cảnh báo sang `throw` — tức chặn sạch mọi đơn nghỉ bù. Lưới thật chỉ có
  một: `_validate_comp_leave_balance`.
